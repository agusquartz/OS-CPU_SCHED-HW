...
public class CPU {
	...
	public void usePage(Process p){
		...
		int pageNum = 1 + (int)(Math.random() * p.getPages());
		Page page = new Page(pageNum, pid);
		//nos fijamos si la página aleatoria esta en ram mediante la PageTable
		if (table.getPresenceValue(pid, pageNum) == null){
			System.out.println("\n\n\nError: PageTable does not contain entry for process " + pid + " and page " + pageNum);
			return; // Exit if the page table entry is missing
		} 
		...
		//System.out.println(ram.getPages().toString());
